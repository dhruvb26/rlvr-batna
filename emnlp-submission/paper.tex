...
First-bid ratio, defined for price scenarios where the opening offer is a scalar, measures how aggressively the buyer anchors below budget \citep{galinsky2001firstoffers, xia2024measuring}. The threshold agent anchors far more aggressively than surplus or the base model: its first-bid ratio falls to ${\approx}\,0.30$ of budget over training (Figure~\ref{fig:first_bid}) and averages $0.36$/$0.40$ on AHP/CRA at evaluation, versus $0.59$/$0.56$ for surplus and $0.95$/$0.92$ for the base model (Table~\ref{tab:fbr_pareto_vs_qwen}), despite the threshold penalty applying only to multi-item deals. Because the agent learns a single policy across all tasks, one might hypothesize that walk-away tolerance learned on multi-item scenarios transfers to price bargaining as deeper anchoring. A controlled ablation rejects this explanation: an agent trained on multi-item tasks alone shows no positive transfer to price bargaining (Appendix~\ref{app:transfer}), indicating that the price gains arise from the direct price-task surplus reward during joint training rather than from cross-task transfer. This direct signal drives the largest single-dataset gain: on AHP, bargained ratio recovers from $-0.05$ (base) to $0.80$.
...
Neither reward sees JI during training, yet the threshold agent transfers at $0.70$ bargained ratio, comparable to frontier models $8$--$200{\times}$ larger (most spanning $0.71$--$0.79$ on JI) and well above surplus ($0.63$). The ability to distinguish good from bad deals, learned on pure price and multi-item tasks, generalizes to JI's hybrid structure without explicit exposure, suggesting that quality-aware negotiation generalizes across formats.
...
% TODO(rebuttal): AHP numbers in Tables 1-2 and 5-6 must be re-run on the
% disjoint data/ahp/test split before this paragraph ships in the revision.
\paragraph{Train/test splits.}
Training and evaluation draw from disjoint scenario pools. CaSiNo and DnD use their standard splits (906 training / 124 evaluation scenarios and 5{,}048 / 526, respectively), as does CraigslistBargains (4{,}020 / 637 usable dialogues). AmazonHistoryPrice provides no canonical split, so we partition its products at the product level into disjoint train and evaluation pools (743 / 187 products; 422 / 93 usable scenarios after price-validity filtering), stratified by category. Job Interview is excluded from training entirely and serves as the held-out transfer benchmark (295 scenarios). Evaluation samples scenarios exclusively from the held-out pools.
...
To isolate the source of the joint agent's price-bargaining gains, we train a threshold agent on multi-item tasks \emph{only} (CaSiNo and DnD, 50/50) and evaluate it on the price tasks it never saw. Because the base model used in the main experiments was retired by the training provider after submission, this ablation uses its official replacement, \model{Qwen3.6-35B-A3B}, with otherwise identical hyperparameters ($\tau{=}0.4$, 80 steps, batch 64, group size 8); all comparisons are against the \emph{same-base} untrained model, served identically, so the only difference is training. AHP is evaluated on the product-disjoint held-out split, and the evaluation opponent matches the main tables (\model{Qwen3-30B-A3B-Instruct-2507}).
...
\section{Uniform vs.\ Selective Thresholding}
\label{app:uniform}

A natural alternative to our selective design is to apply the threshold uniformly, including to price tasks. We quantify why this is harmful post-hoc, using the closed price deals from the main evaluation. Among the threshold agent's \emph{positive-surplus} price deals against the training opponent, only $1\%$ (AHP) and $14\%$ (CRA) fall below $\tau{=}0.4$; against the stronger \model{GPT-5.4} opponent, however, $35\%$ and $45\%$ do---and for the untrained base against the training opponent, $65$--$88\%$ do. Because regulation guarantees these deals have positive surplus, sub-$\tau$ price outcomes against strong opposition are frequently the \emph{best achievable} result (Appendix~\ref{app:frontier}: even the best agent reaches only $0.32$--$0.35$ bargained ratio on price vs.\ \model{GPT-5.4}), not avoidable concessions. A uniform threshold would assign these optimal thin-margin agreements the same penalty as genuine failures, pushing the policy toward the deadlock regime observed by \citet{liu2026instructing}. By contrast, on multi-item tasks filtering sub-$\tau$ deals \emph{improves} quality and Pareto efficiency while deal rates recover (\S\ref{sec:reward}, Table~\ref{tab:results-qwen}), indicating that sub-$\tau$ multi-item deals are predominantly avoidable. This asymmetry is the empirical basis for selective thresholding.
